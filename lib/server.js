...
var fs = require('fs');
var cah_json = JSON.parse(fs.readFileSync('public/a_to_a.json', 'utf8'));
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('disconnect', function () {
    ...
    if (games_list.hasOwnProperty(old_room)) {
      ...
      socket.broadcast.to(old_room).emit('update users bad?', {
  ...
  socket.on('entered room', function (data) {
    ...
    socket.broadcast.to(socket.room).emit('update users bad?', {
    ...
    socket.broadcast.to(game_id).emit('update users', {
  ...
  socket.on('czar has chosen', function (selected) {
    for (var key in chosen_cards[socket.room]) {
      if (chosen_cards[socket.room].hasOwnProperty(key)) {
  ...
  socket.on('new white cards', function (number) {
    var cards_list = [];
